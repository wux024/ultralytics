...
if [ -z "$selected_models" ]; then
    ...
    models=("yolov8n-pose-cspnext-rt.yaml" "yolov8s-pose-cspnext-rt.yaml" "yolov8m-pose-cspnext-rt.yaml" "yolov8l-pose-cspnext-rt.yaml" "yolov8x-pose-cspnext-rt.yaml")
else
    # Process selected models
    for model_code in "${selected_models[@]}"; do
        case $model_code in
            n)
                models+=("yolov8n-pose-cspnext-rt.yaml")
                ;;
            s)
                models+=("yolov8s-pose-cspnext-rt.yaml")
                ;;
            m)
                models+=("yolov8m-pose-cspnext-rt.yaml")
                ;;
            l)
                models+=("yolov8l-pose-cspnext-rt.yaml")
                ;;
            *)
